# hyperlocal-spotlight Specification

## Purpose

Defines the hyperlocal spotlight: a single global slot holding at most one editor-picked article, settable only from the article create/update endpoints rather than a separate curation screen, its automatic fallback to the most recently published article when no pick is set or the pick is not publicly visible, its permission-gated admin read, and the public read that composes the resolved article for the home page.
