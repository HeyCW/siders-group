...
- [ ] 6.15 Test that deleting a curated article removes its entry via `ON DELETE CASCADE`, leaving no dangling reference — **not verified**: this is a live-Postgres foreign-key behavior with no equivalent testable in a fake repository, and no live database is available in this sandbox (same constraint `add-news-management-system`'s own implementation PR recorded for its DB-dependent tasks). The schema declares `ON DELETE CASCADE` (`packages/db/src/schema/homeCuration.ts`); actual cascade behavior needs verification against a real database before this is checked off
...
- [ ] 7.2 Confirm `auditAuthorizationDeclarations` passes at boot — every new route declares a permission or `requirePublic()` — both `curationRoutes` handlers declare `requirePermission('news.manage')` and `publicHomeRoutes`' declares `requirePublic()`; boot-time confirmation needs a running server with a reachable database, unavailable in this sandbox
- [ ] 7.3 Confirm no new rows exist in `app.permissions` after migrating — the migration (`0002_broad_hobgoblin.sql`) contains no `INSERT` into `app.permissions`, confirmed by inspection; running it against a live database to confirm at the row level is unavailable in this sandbox
