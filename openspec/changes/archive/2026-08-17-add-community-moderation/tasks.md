...
- [x] 3.6 `moderation.service.test.ts` (36 tests) and `moderation.mapper.test.ts` (16 tests), 52
      ...
      **Not covered by any test, and not coverable without a live database:** the keyset cursor's
      ...
      actual runtime decoding — specifically that `count(*)` (bigint) and `array_agg(...::text)`
      (a cast enum array) arrive as a `number` and a `string[]` respectively rather than the raw
      strings node-postgres would otherwise hand back, since these are Drizzle/Postgres driver
      behaviors no fixture-based unit test exercises; and the RLS default-deny posture on
      `moderation_actions` and `comment_reports`. All three are exercised by task 5.3.
      (A first version of `reportAggregateSubquery` shipped without `.mapWith(Number)` and without
      the `::text` cast — both compiled and passed every unit test, since the TypeScript
      annotations asserted the post-decode shape without anything exercising the decode itself.
      Caught and fixed in review before task 5.3 could have caught it against a real database.)
