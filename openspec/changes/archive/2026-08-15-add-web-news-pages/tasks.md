...
- [x] 9.2 Manual verification of all four routes against a local `apps/api` with seeded categories/articles/reels/curation — done: Postgres 15 in Docker, migrations + a throwaway dev content seed applied, `apps/api` and `apps/web` run locally, all four routes (`/`, `/news`, `/news/[slug]`, `/contact`) fetched and their SSR output checked against the prototype's copy/structure (masthead, hero, stats band, Guide of the Week, showcase, reels captions, footer, news filters, article kicker/byline/body/drop-cap/engagement bar, contact blocks) — all present. Pixel-level style comparison at specific breakpoints still not done (no browser screenshot tool available in this environment), but this closes the real-data rendering gap 9.2 originally flagged.
...
- [x] 9.4 Verify `/news?category=<slug>` is a shareable, reloadable URL — done: `curl http://localhost:3000/news?category=kuliner` against the live stack returns 200 with only the `kuliner`-tagged article in the SSR output and the others correctly excluded, confirming the filter is real server-side state driven by the URL, not client-only
