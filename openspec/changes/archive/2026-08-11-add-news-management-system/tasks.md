...
- [x] 2.9 The API now asserts its own database role has `BYPASSRLS` at boot (`assertDatabaseRoleBypassesRls` in `apps/api/src/lib/assertDatabaseRole.ts`, called from `server.ts`'s startup path before `app.listen`) and refuses to start otherwise, rather than silently serving empty results — **not yet exercised against a live database in this sandbox** (no `DIRECT_URL`); confirm it passes (or write the equivalent RLS policy) when applying the migration against the real Supabase instance
...
- [x] 10.6 Image insertion is implemented per the current spec: uploads through the admin media endpoint and references the returned media record (verified live), type/size rejections surface via the server's real error message. The extended `Image` node's schema carries `width`/`align`/`caption` attributes that `sanitizeHtml.ts` renders, for future use — the article-editor spec was amended to drop the resize-handle-drag and caption-field scenarios, since no NodeView UI for either exists; building that UI (drag-to-resize handle, alignment picker, caption input) is tracked as a follow-up, not a gap against the current spec
...
- [x] 13.1 Unit tests covering the service-layer logic in every scenario that doesn't require a live database: `article.service.test.ts` (19), `category.service.test.ts` (8), `tag.service.test.ts` (5), `media.service.test.ts` (5), `scheduledPublishWorker.test.ts` (4), `sanitizeHtml.test.ts` (18, one per block type + disallowed-markup cases), `mediaStorage.test.ts` (11), `revalidate.test.ts` (4), `slugify.test.ts` (7, added by VERIFICATION_FIXES.md - Fix 1), plus contract-schema tests (`article.test.ts`, 16). **Not covered**: repository-level Drizzle query correctness (needs a live DB — see 13.7/13.10/13.11), and the article-editor UI scenarios beyond what was verified by hand in a real browser (bubble menu, slash command open/filter/dismiss — see the session's live-testing notes; no automated component test was written for these)
...
- [ ] 13.11 Needs a live DB — this is the single most important scenario in the whole change (design.md's "read-time fallback") and the one most worth writing first once a database is available. `publiclyVisible()` in `article.repository.ts` is structurally guaranteed to be the only definition of the predicate (both `listPublished` and `findPublishedBySlug` call the same private function), which a code reviewer can confirm today, but nothing here executes it against real rows. Also cover, once a database exists: a `published` row with a null `published_at` is correctly *excluded* rather than reaching the mapper and 500ing (VERIFICATION_FIXES.md - Fix 3; the `isNotNull(articles.publishedAt)` guard added to the `published` branch)
