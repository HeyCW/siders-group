...
- [x] 7.7 Fix the concurrent-replace race found during verification — `curation.repository.ts`'s `replace()` caught only foreign-key violations, so two overlapping `PUT`s could both pass their `DELETE` before either `INSERT`ed, and the second `INSERT` then collided with the first's rows on `home_curation_pkey` or `home_curation_position_unique`: an unhandled `23505` surfacing as a 500, not the last-write-wins `design.md` promised. Fixed by adding `LOCK TABLE app.home_curation IN EXCLUSIVE MODE` as the transaction's first statement, forcing the second writer's `DELETE` to run against a fresh snapshot that already includes the first writer's committed rows. **Verified against a live Postgres 16**: the same two-connection race that reliably produced a `23505` before the fix now completes both writes cleanly, with the second fully overwriting the first
- [x] 7.8 Correct `design.md`'s two `GET /public/home` references (lines 83, 122 as originally written) to `GET /home`, matching the actual mount path and the convention already followed by `/articles`, `/categories`, `/tags`. Added a one-line note explaining the convention, and updated the "Writes replace the whole list" walkthrough and "Accepted trade-off" note to describe the `LOCK TABLE` mechanism from 7.7 rather than an unenforced promise
- [x] 7.9 Add two scenarios closing spec gaps identified during verification: "Concurrent replacements do not fail" (under "Curation is replaced as a whole list") and "More curated articles than the limit truncates rather than overflows" (under "Public homepage feed composes curated picks with chronological backfill") — both were previously true in the implementation and tested, but unstated in the spec, which is exactly why 7.7's race was never caught by spec-driven test coverage
- [x] 7.10 Fix a deadlock in `replace()` found in post-implementation review: taking `LOCK TABLE app.home_curation IN EXCLUSIVE MODE` as the transaction's *first* statement (7.7's fix) put a curation replace's lock order out of step with a concurrent article hard-delete's `ON DELETE CASCADE` — the delete locks the article row first and only then needs `home_curation`, so a replace that took the table lock before touching `articles` could deadlock (`40P01`) against a delete of *any* article, curated or not, surfacing as an unhandled 500. **Reproduced against a live Postgres 16**: a concurrent replace and an unrelated article delete deadlocked reliably. Fixed by moving the existence check to a `SELECT ... FOR KEY SHARE` on the submitted article ids *before* the table lock, matching the delete path's order — this doubles as the existence validation, so an unknown id is now rejected directly rather than via `23503` translation. **Re-verified against a live Postgres 16**: the same scenario now completes both transactions without error. Closes the spec gap with a new scenario, "A concurrent article deletion does not deadlock a replacement" (under "Curation is replaced as a whole list") — unstated before this fix, the same gap-then-close pattern as 7.7/7.9
- [x] 7.11 Add a "Feed request limit" requirement to the spec, mirroring `public-news-api`'s "List pagination": default and cap were already implemented and tested (`curation.ts`'s `homeFeedQuerySchema`, `curation.test.ts`), but unstated in `spec.md`, which only described the limit's effect on composition and never its own validation
- [x] 7.12 Correct `design.md`'s stale claims found in post-implementation review: "`UNIQUE(position)` ... is never contended" no longer held once 7.7 established two replaces *do* contend on `home_curation`; narrowed to "never contended by the replacement itself." "Positions are contiguous" contradicted 6.15's own finding (`0, 2` after a cascade delete); corrected to "zero-based ... but not guaranteed contiguous." Updated the `PUT /admin/curation` walkthrough and added a decision note for 7.10's lock-order fix. Corrected `curation.ts`'s `MAX_HOME_CURATION_ENTRIES` comment, which cited `design.md - "Curated list validation"` — that heading has always lived in `spec.md`, not `design.md`
