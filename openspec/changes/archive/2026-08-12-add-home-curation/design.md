...
{ "articleIds": ["…a", "…b", "…c"] }          ← the client never sends a position
  ...
  BEGIN
    SELECT id FROM app.articles WHERE id IN (a,b,c) FOR KEY SHARE   ← every id exists, or reject
...
- **The table lock is load-bearing, not defensive filler.** Without it, two overlapping `PUT`s can both execute their `DELETE` before either `INSERT`s — under READ COMMITTED, a `DELETE` only removes rows visible as of its own statement snapshot, so the second `DELETE` does not see the first transaction's not-yet-committed rows. The second `INSERT` then collides with the first's on `home_curation_pkey` or `home_curation_position_unique`, and an unhandled `23505` reaches the caller as a 500 for a request that, from the editor's side, looked identical to a normal save. `LOCK TABLE ... IN EXCLUSIVE MODE` forces the second writer to wait for the first to commit, so its `DELETE` runs against a fresh snapshot that already includes the first writer's rows — the second write then cleanly and fully overwrites the first. Verified empirically against a live Postgres instance: without the lock, two concurrent replaces reliably produced a `23505`; with it, the second write always completed cleanly.
- **The article row locks come before the table lock, not after — the table lock is not the transaction's first statement.** A hard article delete locks the article's row before it needs to touch `home_curation` (to run the `ON DELETE CASCADE`). Taking `LOCK TABLE app.home_curation` before validating that the submitted ids exist inverts that order: the replace waits on `home_curation` while holding nothing on `articles`, and a concurrent delete of *any* article — not necessarily a curated one — can simultaneously hold the article's row lock while waiting on `home_curation`, a `40P01` deadlock. Reproduced against a live Postgres 16: a curation replace and an unrelated article delete running concurrently deadlocked reliably with the table lock taken first. Selecting the submitted ids `FOR KEY SHARE` before the table lock — the same order the delete path takes — removes the cycle: the loser of the row lock simply waits for the winner to commit, and the row-lock query doubles as the existence check validation already needed, so a submitted id that names no article is rejected there rather than by translating a `23503`.
...
- `UNIQUE(position)` is a correctness guard, not a working constraint — whole-list replacement deletes before it inserts, so it is never contended by the replacement itself.
- Positions are zero-based and assigned from the submitted array index on every write, but are not guaranteed contiguous afterwards: `ON DELETE CASCADE` can remove an interior entry (verified empirically — a 3-entry list left as `0, 2` after deleting the middle article). This is harmless because nothing reads a position's absolute value; the column exists only to make the stored order recoverable via `ORDER BY position`.
