...
_None._ `add-news-management-system` is now implemented and archived (`archive/2026-08-11-add-news-management-system/`), and `public-news-api` exists as a real spec at `openspec/specs/public-news-api/spec.md`. This change consumes that capability's canonical visibility rule and list query layer — including the `excludeIds` parameter it already exposes — without altering its spec, so no delta is recorded against it here.

## Impact

- **Affected code**: `packages/db` (new `home_curation` table + migration), `packages/contracts` (curation request/response Zod schemas), `apps/api/src/modules/curation/**` (new: routes, controller, service, repository, mapper), `apps/api/src/lib/revalidate.ts` (a `/` call on curation write — this file already exists, added by `add-news-management-system`), `apps/admin` (curation screen). `apps/web` is not modified.
- **Dependencies**: this change was drafted stacked behind `add-news-management-system`. That change is now implemented and merged into `main` (`apps/api/src/modules/articles`, the `excludeIds`-capable list query in `article.repository.ts`, and `apps/api/src/lib/revalidate.ts` all exist there today), so the prerequisite in `tasks.md` §0 is satisfied and implementation of this change can begin.
...
- **Known inaccuracy in the archived record**: the now-archived `archive/2026-08-11-add-news-management-system/design.md` justifies `excludeIds` by saying the homepage curation change "fills uncurated slots from this endpoint", i.e. that the consumer composes client-side. This change composes server-side instead (see `design.md` — "Composition happens in the API"). The parameter is still required and still used, just by this change's own composed endpoint rather than by `apps/web` directly. Archived changes are a historical snapshot and are not edited after archiving, so that sentence is left as written there; this proposal's `design.md` is the authoritative description of how `excludeIds` is actually used.
