...
- [x] 3.4 Test the partner module's rules, each where it actually lives rather than all in one file: `partner.service.test.ts` (logo must reference an existing media record, on create and update; not-found on update/delete; every write requests revalidation; public listing excludes inactive); `partner.repository.test.ts` (`isExactPartnerIdSet` — the reorder set rule, extracted pure so it is testable without a database, covering omitted ids, unknown ids, right-length-wrong-member, duplicates, and order-independence); `partner.service.revalidation.test.ts` (revalidation failure does not fail the write — runs the real `revalidateHomePath` over a failing `fetch`, since a mocked one cannot establish this); `packages/contracts/src/partner.test.ts` (logo required at creation, website URL must be a valid absolute http(s) URL, `sortOrder` rejected from both request shapes). **Not covered by any test, and not coverable without a live database:** reorder atomicity and delete's order self-healing both live in SQL — the transaction boundary and the absence of an ordering table respectively. Verified by inspection and by task 7.2's manual database exercise.
...
- [x] 8.2 Manually verify in a browser: create several partners with differently-shaped logos, confirm the ticker scrolls, pauses on hover and on keyboard focus, and that reduced-motion (OS/browser setting) shows the static fallback with every partner visible. Verified by driving a production `next build` + `next start` in real Chromium with three partners at 160×60, 90×60 and 200×60 — 19 assertions, all passing: the `marquee` animation applies and the track measurably translates; one half of the track is 3840px against a 1040px container, so few partners still loop seamlessly; hover pauses and releases; the ticker holds exactly one reachable link per partner with every duplicate `aria-hidden` + `tabindex="-1"`; logos render `object-contain` with `filter: none`; and under emulated `prefers-reduced-motion: reduce` the ticker is hidden, the static grid shows each partner exactly once, nothing animates, and all three links are keyboard-focusable.
  - **This found a real bug, now fixed.** With `focus-within` only pausing the animation, a link tabbed to mid-cycle measured **0 of 160px visible** inside the `overflow-hidden` clip — the transform had carried it out of the clip region, and the browser's scroll-into-view had nothing to act on (`scrollLeft` stayed 0), so the focus target was simply invisible. `focus-within:[animation:none]` snaps the track back to `translateX(0)`, where the tabbable copies live; the same probe now measures **160 of 160px visible** at every point in the cycle. Hover still pauses in place.
