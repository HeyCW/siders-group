 ...
 */
function resolveSsl(databaseUrl: string, nodeEnv: string | undefined) {
  const sslmode = new URL(databaseUrl).searchParams.get('sslmode');
  if (sslmode === null || sslmode === 'disable') {
    if (nodeEnv === 'production') {
      throw new Error(
        "DATABASE_URL must set sslmode (e.g. 'require') in production — refusing to connect to MySQL in plaintext.",
      );
    }
    return undefined;
  }
...
/**
 * mysql2's `Pool.on('connection', ...)` is declared (in `promise.d.ts`) as handing back the
 * promise-wrapped connection, but the core pool actually emits the raw callback-style connection
 * directly (`mysql2/lib/base/pool.js`'s `this.emit('connection', connection)` — never wrapped;
 * `mysql2/lib/promise/inherit_events.js` just re-emits whatever the core pool sent). This narrow
 * shape describes what is actually there instead of trusting the misleading declared type.
 */
...
export function getDb(env: DbEnv) {
  ...
  pool.on('connection', (connection) => {
    const raw = connection as unknown as CallbackQueryable;
    raw.query('SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED', (err) => {
      if (err) console.error('failed to set MySQL session isolation level', err);
    });
    raw.query("SET SESSION time_zone = '+00:00'", (err) => {
      if (err) console.error('failed to set MySQL session time zone', err);
    });
  });
