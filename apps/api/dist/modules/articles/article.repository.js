...
import { articles, articleCategories, categories, media, users, anakUsaha, newId, } from '@siders/db';
...
function translateArticleWriteError(err) {
    if (isUniqueViolationOn(err, SLUG_CONSTRAINT))
        return slugConflictError();
    if (isForeignKeyViolation(err)) {
        const constraint = violatedConstraint(err);
        if (constraint?.includes(FEATURED_MEDIA_CONSTRAINT))
            return invalidFeaturedMediaError();
        if (constraint?.includes(ANAK_USAHA_CONSTRAINT))
            return invalidAnakUsahaError();
...
function publiclyVisible(now) {
    ...
    and(eq(articles.status, 'published'), isNotNull(articles.publishedAt)), and(eq(articles.status, 'scheduled'), lte(articles.publishedAt, now)));
}
/**
 ...
 */
export function isPubliclyVisible(row, now) {
    if (row.status === 'published')
        return row.publishedAt !== null;
    if (row.status === 'scheduled')
        return row.publishedAt !== null && row.publishedAt.getTime() <= now.getTime();
    return false;
}
...
async function attachRelations(db, rows) {
    if (rows.length === 0)
        return [];
...
export function createArticleRepository(db) {
    ...
    return {
        async create(input) {
            try {
                ...
                if (!row)
                    throw new Error('article missing immediately after create');
        ...
        async update(id, input) {
            ...
            const row = await findRowById(id);
            if (!row)
                throw new Error('article missing immediately after update');
            const [withRelations] = await attachRelations(db, [row]);
            if (!withRelations)
                throw new Error('article missing immediately after update');
            return withRelations;
        ...
        async updateStatus(id, status, publishedAt) {
            ...
            const row = await findRowById(id);
            if (!row)
                throw new Error('article missing immediately after status update');
            const [withRelations] = await attachRelations(db, [row]);
            if (!withRelations)
                throw new Error('article missing immediately after status update');
            return withRelations;
        ...
        async promoteScheduled(id, now) {
            ...
            const [result] = await db
                ...
                .where(and(eq(articles.id, id), eq(articles.status, 'scheduled'), lte(articles.publishedAt, now)));
        ...
        async listPublished(filter) {
            ...
            if (filter.categorySlugs && filter.categorySlugs.length > 0) {
                conditions.push(inArray(articles.id, db
            ...
            if (filter.anakUsahaSlugs && filter.anakUsahaSlugs.length > 0) {
                conditions.push(inArray(articles.anakUsahaId, db.select({ id: anakUsaha.id }).from(anakUsaha).where(inArray(anakUsaha.slug, filter.anakUsahaSlugs))));
            }
