 ...
 */
export function createGuidePickService(repository, revalidateEnv, logger) {
    return {
        async create(input) {
            let row;
            try {
                ...
                if (isForeignKeyViolation(err))
                    throw invalidMediaReferenceError(err);
                throw err;
            }
        ...
        async update(id, input) {
            ...
            if (!existing)
                throw new AppError('Guide pick not found', 404, 'not_found');
            let updated;
            try {
 ...
 */
export function createPublicGuidePickService(repository) {
    return {
        listPublic() {
