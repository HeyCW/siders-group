...
export function staffRoutes(db, env) {
    const router = Router();
    const staffRepository = createStaffRepository(db);
    const authService = createAuthService(createSessionRepository(db), env);
    const service = createStaffService(db, staffRepository, (subjectType, subjectId) => authService.revokeAllForSubject(subjectType, subjectId), (subjectType, subjectId, exceptSessionId) => authService.revokeAllForSubjectExcept(subjectType, subjectId, exceptSessionId));
    ...
    router.post('/me/password', requireStaff({ allowPendingPasswordChange: true }), passwordChangeRateLimiter(), controller.changePassword);
    return router;
}
