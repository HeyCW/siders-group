...
export function toStaffAccountResponse(row) {
    return {
        id: row.id,
        email: row.email,
        name: row.name,
...
export function toStaffCreateResponse(row, temporaryPassword) {
    return { ...toStaffAccountResponse(row), temporaryPassword };
}
export function toStaffResetResponse(temporaryPassword) {
