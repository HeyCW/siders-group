 ...
 */
export function staffLoginRateLimiters() {
    return [
        rateLimit({
...
export function authRoutes(db, env) {
    const router = Router();
    ...
    router.post('/refresh', requirePublic(), refreshRateLimiter(), controller.refresh);
    ...
    router.get('/csrf', requirePublic(), csrfBootstrapRateLimiter(), async (req, res, next) => {
        try {
            let sessionId = null;
            if (req.auth) {
    ...
    // revoked in bulk").
    router.post('/sessions/revoke-all', requirePermission('settings.manage'), async (_req, res, next) => {
        try {
    ...
    router.post('/staff/login', requirePublic(), ...staffLoginRateLimiters(), async (req, res, next) => {
        try {
