 ...
 */
export function encodeCursor(source) {
 ...
 */
export function planReaderModeration(existing, request, actorId, now) {
    const patch = {};
    const logEntries = [];
    ...
    if (request.mutedUntil !== undefined) {
        ...
        if (requested !== null && requested.getTime() <= now.getTime())
            throw invalidMuteDurationError();
