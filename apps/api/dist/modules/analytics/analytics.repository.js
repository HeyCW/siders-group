...
import { articles, articleCategories, articleViewsDaily, readers, homeCuration, } from '@siders/db';
 ...
 */
export function jakartaWeekStart(instant) {
...
export function jakartaDateLabel(instant) {
    const shifted = new Date(instant.getTime() + JAKARTA_OFFSET_MS);
...
function countWhere(condition) {
    return sql `count(case when ${condition} then 1 end)`.mapWith(Number);
}
export function createAnalyticsRepository(db) {
    return {
        ...
        async getCadence(now) {
            ...
            const bucketExpr = sql `date_format(date_sub(date(${jakartaLocal}), interval weekday(${jakartaLocal}) day), '%Y-%m-%d')`;
            const rows = await db
        ...
        async getContentDebt() {
            const uncategorizedCondition = notExists(db.select({ one: sql `1` }).from(articleCategories).where(eq(articleCategories.articleId, articles.id)));
        ...
        async getUpNext(now) {
            const dueSoonRows = await db
                ...
                .from(articles)
                .where(and(eq(articles.status, 'scheduled'), gte(articles.publishedAt, now), lte(articles.publishedAt, new Date(now.getTime() + DUE_SOON_WINDOW_MS))))
        ...
        async getReadership(now) {
            ...
            const totalViews = sql `coalesce(sum(${articleViewsDaily.views}), 0)`;
            const [totals, topArticles] = await Promise.all([
                db
                    .select({
                    last7dViews: totalViews.mapWith(Number),
                    last7dUniqueViews: sql `coalesce(sum(${articleViewsDaily.uniqueViews}), 0)`.mapWith(Number),
                })
