import type {
  CommentQueueRow as CommentQueueRowDto,
  CommentReportResponse,
  ModerationActionResponse,
  ReaderQueueRow as ReaderQueueRowDto,
} from '@siders/contracts';
import type {
  CommentQueueRow as CommentQueueRowSource,
  CommentReportRow,
  ModerationActionRow,
  ReaderQueueRow as ReaderQueueRowSource,
 ...
 */
export function toCommentQueueRow(row: CommentQueueRowSource): CommentQueueRowDto {
  return {
    id: row.id,
    body: row.body,
    status: row.status,
...
export function toReaderQueueRow(row: ReaderQueueRowSource): ReaderQueueRowDto {
  return {
    id: row.id,
    name: row.name,
    email: row.email,
...
export function toModerationActionResponse(row: ModerationActionRow): ModerationActionResponse {
  return {
    id: row.id,
...
export function toCommentReportResponse(row: CommentReportRow): CommentReportResponse {
  return {
    id: row.id,
