 ...
 *  stored on the row, matching how `media.mapper.ts` derives `url`. No `url` field for the
 *  reel's own source video is ever produced, because none is ever stored
...
export function toReelResponse(env: { MEDIA_PUBLIC_BASE_URL: string }, row: ReelRow): ReelResponse {
  return {
    ...
    posterUrl: publicUrlFor(env, row.posterStoragePath),
    caption: row.caption,
    status: row.status,
