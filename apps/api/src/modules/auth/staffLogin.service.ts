...
export interface StaffLoginService {
  login(email: string, password: string): Promise<{ subjectId: string }>;
}
 ...
 */
export function createStaffLoginService(repository: Pick<StaffRepository, 'findByEmail'>): StaffLoginService {
  return {
