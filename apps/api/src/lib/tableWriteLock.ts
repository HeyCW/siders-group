...
/**
 * Deliberately excludes the bare `Database` pool: `GET_LOCK`/`RELEASE_LOCK` are connection-scoped
 * (see the file comment below), so acquire and release must run on the one pooled connection
 * Drizzle pins for a `db.transaction(...)` callback. Passing a pool here would silently acquire
 * and release on two different connections — a compile-time error is cheaper than that surfacing
 * only under concurrency in production.
 */
export type LockExecutor = Parameters<Parameters<Database['transaction']>[0]>[0];
...
function lockConflictError(table: string): AppError {
  return new AppError(
...
function assertLockNameFits(lockName: string): void {
...
export async function withTableWriteLock<T>(
  tx: LockExecutor,
  table: string,
  fn: () => Promise<T>,
  ...
  const [rows] = (await tx.execute(sql`select get_lock(${lockName}, ${timeoutSeconds}) as acquired`)) as unknown as [
