...
const HEADING_TAGS: Record<number, string> = { 1: 'h1', 2: 'h2', 3: 'h3' };
...
function nodeArray(content: unknown): PMNode[] {
  if (!Array.isArray(content)) return [];
  return content.filter((n): n is PMNode => typeof n === 'object' && n !== null);
}

function markArray(marks: unknown): PMMark[] {
  if (!Array.isArray(marks)) return [];
  return marks.filter((m): m is PMMark => typeof m === 'object' && m !== null);
}

/** Wraps already-escaped text in every allowlisted mark it carries. Unknown marks are dropped. */
function renderMarks(innerHtml: string, marks: PMMark[]): string {
...
function renderCodeText(nodes: PMNode[]): string {
  return nodes
 ...
 */
function renderVideo(node: PMNode): string {
