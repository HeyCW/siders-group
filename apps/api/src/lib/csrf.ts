 ...
 */
export function issueCsrfToken(env: CsrfEnv, sessionId: string): string {
...
export interface CsrfTokenPayload {
  /** The session this token was issued for — compared against the caller's `sid` claim. */
  sessionId: string;
}

/** Returns the bound session id, or null when the signature does not verify. */
export function verifyCsrfToken(token: string, env: CsrfEnv): CsrfTokenPayload | null {
...
export function clearCsrfCookie(res: Response, options: Pick<CookieOptions, 'secure' | 'domain'>): void {
 ...
 */
export function createCsrfMiddleware(env: CsrfEnv) {
