...
import type { ReelResponse, ReelStatus } from '@siders/contracts';
...
export function ReelLibraryPage() {
  ...
  const [updateState, runUpdate] = useAsyncAction((id: string, status: ReelStatus) =>
    reelsApi.update(id, { status }),
  );
        ...
        {reels.map((reel) => (
          <li key={reel.id} className="flex items-center justify-between gap-3 px-3 py-2">
            ...
            <div className="flex shrink-0 items-center gap-2">
              <select
                value={reel.status}
                onChange={(e) => handleStatusChange(reel.id, e.target.value as ReelStatus)}
